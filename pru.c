...
int Menu(int flag, int estudiantesmat[5][7][2])
{
	...
	while (Menu)
	{
		...
		case 1:
			...
			for (int i = 0; i < 5; i++)
			{
				...
				for (int j = 0; j < 7; j++)
				{
					for (int k = 0; k < 2; k++)
					{
						fprintf(archivo,"El numero de alumnos en %s en el semestre %d es: ", Carreras[j], k + 1);
		...
		case 2:
			...
			fprintf(archivo, "Respuesta: El anio en que ingreso la mayor cantidad de alumnos fue el %d con un total de %d alumnos\n\n", max1 + 1, cantmax1[max1]);
			break;
		case 3:
			Menu = 0;        // Se deja de visualizar el menu
			...
			fprintf(archivo, "Respuesta: La Carrera que recibio la mayor cantidad de alumnos en el ultimo anio es %s con un total de %d alumnos\n\n", Carreras[max2], cantmax2[max2]);
			break;
		case 4:
			Menu = 0;        // Se deja de visualizar el menu
			...
			// Imprime la respuesta
			fprintf("Respuesta: El anio en que la carrera de Ingenieria de Software recibio la mayor cantidad de alumnos fue el %d con %d alumnos\n\n", max3 + 1, cantmax3[max3], archivo);
			break;
		case 5:
			Menu = 0; // Se deja de visualizar el menu
		...
		getchar();
		fprintf(archivo, "Presione ENTER para regresar al menu...");
		getchar();
	}
	fclose(archivo);
